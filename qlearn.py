...
class Qlearn:
    ...
    def next(self, position):
        ...
        self.updateQValue(self.Q, 0.8, current_position, action, score, next_state)
        return next_state, action if action == 1 else -1
    
    def getScore(self, state, action):
        return self.R[state][action]
    
    def updateQValue(self, Q, lr, current_position, action, score, next_state):
        Q[current_position][action] = (1-lr) * Q[current_position][action] + lr *(score + self.gamma * max(Q[next_state,:]))
